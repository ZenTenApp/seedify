...
var (
	...
	brainBunkerReuseKeyPassphrase bool
...
with a space. Check your HISTCONTROL or HIST_IGNORE_SPACE settings.`,
  ...
  seedify ~/.ssh/id_ed25519 --to-jks --bits 2048 --jks-alias zenten --jks-validity 10000 --output zenten-release.jks`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if brainBunkerReuseKeyPassphrase && seedPassphrase == "" {
				return errors.New("--brain-bunker-reuse-key-passphrase requires --brain-bunker")
			}
...
func init() {
	...
	rootCmd.PersistentFlags().BoolVar(&brainBunkerReuseKeyPassphrase, "brain-bunker-reuse-key-passphrase", false, "Protect the ephemeral SSH private key generated by --brain-bunker with the source SSH key passphrase instead of the brain bunker secret")
...
func generateBraveSyncPhrase(path string, seedPassphrase string) (string, error) {
	...
	ed25519Key, _, err = activateBrainBunkerSSHKey(ed25519Key, bts, seedPassphrase, sourcePass)
	if err != nil {
		return "", err
	}
	seedPassphrase = ""
...
func generatePhrasesOutput(keyPath string, seedPassphrase string) error {
	...
	privateKeyPEM := bts
	ed25519Key, privateKeyPEM, err = activateBrainBunkerSSHKey(ed25519Key, privateKeyPEM, seedPassphrase, sourcePass)
	if err != nil {
		return err
	}
	seedPassphrase = ""
...
func activateBrainBunkerSSHKey(sourceKey *ed25519.PrivateKey, sourcePrivateKeyPEM []byte, brainBunker string, sourcePass []byte) (*ed25519.PrivateKey, []byte, error) {
	if brainBunker == "" {
		return sourceKey, sourcePrivateKeyPEM, nil
	}

	keyPassphrase := []byte(brainBunker)
	if brainBunkerReuseKeyPassphrase {
		if len(sourcePass) == 0 {
			return nil, nil, errors.New("--brain-bunker-reuse-key-passphrase requires the source key to be password-protected")
		}
