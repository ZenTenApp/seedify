...
| `--brain-bunker-key-passphrase` | Protect the ephemeral SSH private key generated by `--brain-bunker` with this passphrase instead of the source SSH key passphrase |
...
- **Brain bunker**: Use `--brain-bunker` to derive an ephemeral Ed25519 SSH key
  ...
  source SSH key passphrase. Add `--brain-bunker-key-passphrase` to encrypt
  that ephemeral key with a different passphrase; the bunker secret is still
  used for derivation.
